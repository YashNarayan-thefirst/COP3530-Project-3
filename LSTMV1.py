...
async def evaluate_model(model, X_test, y_test):
    test_loss = 0
    for i in range(0, len(X_test), model.batch_size):
        ...
        loss = await model.eval_step(x_batch, y_batch)
        test_loss += loss
...
async def train_model():
    ...
    for epoch in range(epochs):
        ...
        for i in range(0, len(X_train), custom_lstm.batch_size):
            ...
            loss = await custom_lstm.train_step(x_batch, y_batch, lr=lr)
            epoch_loss += loss

        val_loss = await evaluate_model(custom_lstm, X_test, y_test)
...
async def main():
        await train_model()  # Train the model
        final_loss = await evaluate_model(custom_lstm, X_test, y_test)
